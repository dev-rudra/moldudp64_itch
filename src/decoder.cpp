...
bool parse_moldudp64_header(const uint8_t* packet, int packet_len, MoldHeader* out) {
    ...
    const uint8_t* seq_ptr = packet + 10;
    out->sequence_number = read_u64_big_endian(seq_ptr);

    const uint8_t* count_ptr = packet + 10 + 8;
    out->message_count = read_u16_big_endian(count_ptr);
